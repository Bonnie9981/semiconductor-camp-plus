...
| `PanelSpec` | `core/types.ts` | 「我要玩家做什麼」的宣告式描述，共三種 |
...
| kind | 用途 | 已用於 | 適合 |
| --- | --- | --- | --- |
| `choice` | 從 N 個選項挑 M 個 | 沉積的製程選擇（備援）、正負光阻、顯影液、蝕刻方式 | 選一個東西 |
...
> 曾經有第四種 `mix`（用 ＋/− 調份數），但五關都沒用到 —— 配比全部改成「動手把
> 藥瓶倒進杯子」的 `pour`。已於發表整理時移除；要調數值的互動請優先考慮做成手勢。
...
| **「拖對了 → 子步驟推進」的手勢互動流程沒有測試** | `stage-machine.mjs` 已有 onEnter + 8 幀空跑冒煙、choice 面板驅動的三維分支測試，但「捏著藥瓶拖進正確的槽 → 子步驟推進」這種靠座標命中的流程仍未測 | 這類迴歸只能靠手動玩 | 要餵「有座標的 `HandFrame` 序列」並讓假的 `desk.geometry` / hit-box 對得起來；`makeFrameStubs()` 已是起點 |
...
| ~~關卡類別完全沒有自動測試~~ | `scripts/checks/stage-machine.mjs`：① 窄的 `makeStubContext()` 跑完整 StageManager 生命週期 + 預設路線 `devComplete()` 鏈 + `buildResult()`；② `makeFrameStubs()` + permissive 2D context 替身跑每關 `onEnter` + 8 幀 `onFrame` 冒煙；③ 透過真實 choice 面板的 `onToggle` 驅動 method / tone / etch 三維分支（`BaseStage.goToSubForTest()` 跳到選擇子步驟）。原本「零真實關卡覆蓋」 |
