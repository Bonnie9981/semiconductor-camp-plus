...
左側面板有一個**計時器**，從進入遊戲開始算、五道製程全部完成時定格。

五道製程全部走完後會進入**結業證書**畫面：自動拍一張照片，和你親手做出來的
晶圓圖案、**總花費時間**與各步驟的選擇一起排成一張證書，
可下載 **PDF 證書**、**晶圓 PNG** 與 **3D 模型 STL**。
...
例如燒杯高度是 `clamp(min(bandH × 0.46, sceneW × 0.46), 96, 196)` ——
高度與寬度都不足時取小者，但永遠不小於可辨識的下限。

#### 同一段空間要一起分配，不能各自算

最容易踩的坑不是「元素太小」而是「元素互相壓到」。
RCA 檯面原本燒杯與藥瓶層板**各自獨立**算高度：燒杯吃 `height × 0.29`，
層板只留 `clamp(height × 0.045, 14, 40)` 給自己。視窗一矮，間隙縮到 27px，
但藥瓶名稱掛在瓶底下方，佔掉 `0.24 × 瓶寬 + 一行字` ≈ 31px —— 名稱就壓進燒杯裡。

修法是把整段垂直空間交給**一個純函式由上往下依序分配**
（`scene/WetBenchLayout.ts` 的 `wetBenchLayout()`，藥液槽則是
`scene/TankBench.ts` 的 `tankBenchGeometry()`）。抽成純函式的附帶好處是
...
常見觸發原因就是 Windows 顯示縮放：1920×1080 在 125% 下回報成 1536×864（支援），
但 1280×1024 在 125% 下變成 1024×819（寬度不足）。
...
### 安全規則：沒有水墊底時不可以混合兩種不同的藥液

在杯子裡**還沒有去離子水**的情況下，讓兩種**不同**的藥液相遇 → 觸發**突沸**：
閃焰、衝擊波、噴濺的液滴，然後整杯報廢。

濃酸、濃鹼與雙氧水彼此混合是劇烈放熱反應。有水墊底時，水的高熱容量能吸熱、
對流也帶得走；沒有水的話，兩種濃藥液的界面局部瞬間到達沸點，
就把高溫強腐蝕的液體整團噴出來。所以實務上的習慣是**先加水，再加其他藥液**。

⚠️ 觸發條件既不是「水沒有第一個倒」，也不是「杯子裡有東西」：

- 空杯倒單一藥液 —— 安全（去氧化層那步只用 1 份 HF，完全不涉及這個風險）
- **同一瓶多倒幾份 —— 安全**（沒有不同的東西可以反應；倒錯量只是比例不對，不會炸）
- 往已有的藥液裡補水 —— 安全，這是正確的補救方向

真正的危險只有一種：**在乾的狀態下讓兩種不同的藥液相遇**。

判定寫成純函式 `isUnsafePour(pouringId, currentIds)`（`data/solutions.ts`），
所以 `npm run check` 驗得到（14 組案例，含「同一瓶多倒」這個曾經誤判的情況）；
動畫在 `scene/Explosion.ts`。
...
開關旁邊還有一顆 **⚡ 標記本關完成**，跳過 `canComplete()` 直接把目前這關標記為 done ——
跳關之後過關條件多半不成立，沒有它就走不完整條流程。

它不只是改狀態旗標：`StageManager.forceCompleteCurrent()` 會先呼叫
`BaseStage.devComplete()`，讓該關把**它對晶圓該做的事**補上，否則後面的關卡與
最終的證書、STL 都會拿到一片空白的晶圓。各關的補法：

| 關卡 | `devComplete()` 做的事 |
| --- | --- |
| RCA 清洗 | 四項污染歸零 |
| 薄膜沉積 | 補上金屬層（CVD 路線再補氧化層） |
| 微影製程 | 補上光阻層；有畫圖就取樣成 `exposedMask`，沒畫就給一組條紋 |
| 顯影 | 呼叫 `wafer.develop()` |
| 蝕刻 | `wafer.etch()` 後剝掉光阻 |

八種組合（正/負光阻 × PVD/CVD × 乾/濕蝕刻）都驗過最終狀態合理，
且正負光阻的蝕刻結果互補。
