...
### 安全規則：沒有水墊底時不可以混合兩種藥液

在杯子裡**還沒有去離子水**的情況下，讓第二種藥液碰到第一種 → 觸發**突沸**：
閃焰、衝擊波、噴濺的液滴，然後整杯報廢。
...
⚠️ 注意觸發條件**不是**「水沒有第一個倒」—— 空杯裡只放一種藥液沒有東西可以反應，
並不危險（去氧化層那步只用 1 份 HF，就完全不涉及這個風險）。
真正的危險是**在乾的狀態下讓兩種藥液相遇**。

判定寫成純函式 `isUnsafePour(pouringId, currentIds)`（`data/solutions.ts`），
所以 `npm run check` 驗得到（10 組案例）；動畫在 `scene/Explosion.ts`。
